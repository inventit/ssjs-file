...
var script = require('path').resolve('./src/fetch-file-result.js');
...
module.exports = nodeUnit.testCase({
  ...
  'file fetching result, successful case.' : function(assert) {
    // record state
  ...
  'file fetching result, error case.' : function(assert) {
    // record state
    var context = moat.init(sinon);
    var args = {
      contents: [
  ...
  'file fetching result(2), successful case.' : function(assert) {
    // record state
    var context = moat.init(sinon);
    var args = {
      contents: [
    ...
    assert.equal('/path/to/uid-2.file', contentInfo.sourcePath);
    assert.equal('http://localhost/file2.name', contentInfo.uploadUrl);
